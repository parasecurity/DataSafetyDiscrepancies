...
class FridaMonitor:
    ...
    def clean_frida(self):
        ...
        run_command(download_command,cwd=self.current_dir.parent)
        time.sleep(5)
        # Uncompress the downloaded file
        uncompress_command = ["unxz","utils/frida-server.xz"]
        run_command(uncompress_command,cwd=self.current_dir.parent)
        time.sleep(5)        
        print("Cleaning previous frida-server failed.")

    def push_frida(self):
        process_push = subprocess.Popen(['adb','push','utils/frida-server',"/data/local/tmp"], text=True,cwd=self.current_dir.parent)
        time.sleep(5)
